...
def update(frame, mlp, ax_input, ax_hidden, ax_gradient, X, y):
    ...
    W2 = mlp.W2.flatten()
    b2 = mlp.b2.flatten()
    # Only plot if all weights are non-zero to avoid division errors
    if not np.isclose(W2, 0).any():
        # Create grid
    ...
    ax_input.contour(xx, yy, Z, levels=[0.5], colors='k', linewidths=2)
    # Plot filled contours for class regions
    ax_input.contourf(xx, yy, Z, levels=[0, 0.5, 1], alpha=0.5, cmap='coolwarm')
    # Plot training data
    ax_input.scatter(X[:, 0], X[:, 1], c=y.ravel(), cmap='coolwarm', edgecolor='k', s=40)
    ...
    input_neurons = [(0, y_pos) for y_pos in np.linspace(0.2, 0.8, input_dim)]
    hidden_neurons = [(1, y_pos) for y_pos in np.linspace(0.1, 0.9, hidden_dim)]
    output_neurons = [(2, 0.5)]

    # Plot neurons
    for x, y_pos in input_neurons:
        circle = Circle((x, y_pos), radius=0.05, fill=True, color='red', ec='k')
        ax_gradient.add_artist(circle)
    for x, y_pos in hidden_neurons:
        circle = Circle((x, y_pos), radius=0.05, fill=True, color='yellow', ec='k')
        ax_gradient.add_artist(circle)
    for x, y_pos in output_neurons:
        circle = Circle((x, y_pos), radius=0.05, fill=True, color='purple', ec='k')
        ax_gradient.add_artist(circle)

    # Plot edges from input to hidden layer
    max_grad_w1 = np.max(np.abs(mlp.dW1)) + 1e-8  # Add epsilon to avoid division by zero
    for i, (x1, y1) in enumerate(input_neurons):
        for j, (x2, y2) in enumerate(hidden_neurons):
            weight_grad = mlp.dW1[i, j]
            lw = (abs(weight_grad) / max_grad_w1) * 5  # Scale linewidth
            ax_gradient.plot([x1, x2], [y1, y2], color='gray', lw=lw)

    # Plot edges from hidden to output layer
    max_grad_w2 = np.max(np.abs(mlp.dW2)) + 1e-8  # Add epsilon to avoid division by zero
    for i, (x1, y1) in enumerate(hidden_neurons):
        x2, y2 = output_neurons[0]
        weight_grad = mlp.dW2[i, 0]
        lw = (abs(weight_grad) / max_grad_w2) * 5  # Scale linewidth
        ax_gradient.plot([x1, x2], [y1, y2], color='gray', lw=lw)

    ax_gradient.set_xlim(-0.5, 2.5)
    ax_gradient.set_ylim(0, 1)
    ax_gradient.axis('off')
    ax_gradient.set_title('Network Gradients (Edge Thickness ∝ Gradient Magnitude)')
